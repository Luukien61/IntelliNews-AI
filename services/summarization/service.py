...
class SummarizationService:
    ...
    async def get_or_generate_summaries(
        ...
        # 3. Run models off the asyncio event loop
        logger.info(f"Scheduling summarization models in thread pool for news_id={news_id}")
        summary_short, summary_default = await asyncio.to_thread(
            _generate_summaries_from_text, content_text
        )
    ...
    async def summarize_news(self, news_id: int, force: bool, db: Session) -> NewsSummarizationResponse:
        """
        Generate summaries for a news article and return the response.
        """
        logger.info(f"Summarization request for news_id={news_id}, force={force}")
        
        result, cached = await self.get_or_generate_summaries(
            news_id=news_id,
            db=db,
            force=force,
        )
    ...
    def get_cached_summaries(self, news_id: int, db: Session) -> Optional[dict]:
    ...
    def get_summaries_response(self, news_id: int, db: Session) -> NewsSummarizationResponse:
        """Get cached summaries and return response object."""
        result = self.get_cached_summaries(news_id, db)
        if not result:
            raise ValueError(f"No cached summaries found for news_id={news_id}")
